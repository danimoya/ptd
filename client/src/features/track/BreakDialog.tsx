...
import { BREAK_ICON_NAMES, BUILTIN_BREAKS, iconFor } from "@/components/break-icons";
import { createTemplate, trackKeys, type TemplateRow } from "./api";

/** The full break menu: built-in kinds, the member's own saved tiles, a free-text note, and a way to save a new tile. */
export function BreakDialog({
  ...
  const [draft, setDraft] = useState({ name: "", icon: "coffee", note: "" });

  const customBreaks = templates.filter((t) => t.isBreak);

  const saveTemplate = useMutation({
    mutationFn: () => createTemplate({ name: draft.name, icon: draft.icon, notes: draft.note || draft.name, isBreak: true }),
    onSuccess: () => {
  ...
  const begin = (args: { templateId?: number; label?: string }) => {
    onBreak(args);
    setOpen(false);
    setCustomNote("");
  };

  const tile = (key: string, iconName: string, label: string, onClick: () => void) => {
    ...
    return (
      ...
      >
        ...
        <span className="font-display italic text-sm text-ink text-center leading-tight">{label}</span>
      </button>
    );
  };
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="paper w-[95vw] max-w-[520px] max-h-[90vh] overflow-y-auto rounded-sm border-ink/20">
        <DialogTitle className="font-display font-normal italic text-2xl">A pause from work</DialogTitle>
        <DialogDescription className="font-serif">
          Pick a kind — or tap <span className="italic">Other…</span> for a custom note. Anything running is closed first.
        </DialogDescription>

        <div className="mt-4">
          <div className="eyebrow mb-2">Quick types</div>
          <div className="grid grid-cols-4 gap-2">
            {BUILTIN_BREAKS.map((b) => tile(`bi-${b.icon}`, b.icon, b.name, () => begin({ label: b.note })))}
          </div>
        </div>
        ...
        {customBreaks.length > 0 && (
          <div className="mt-5">
            ...
            <div className="grid grid-cols-4 gap-2">
              {customBreaks.map((t) => tile(`ct-${t.id}`, t.icon ?? "coffee", t.name, () => begin({ templateId: t.id })))}
            </div>
          </div>
        )}
        ...
        <div className="mt-5 border-t border-rule pt-4">
          ...
          <div className="flex gap-2">
            <Input
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter" && customNote.trim()) begin({ label: customNote.trim() });
              }}
              className="h-11 rounded-sm border-ink/30 font-serif"
            />
            <Button
              onClick={() => begin({ label: customNote.trim() || "Recess" })}
              disabled={busy}
        ...
        <div className="mt-5 border-t border-rule pt-4">
          {!saveMode ? (
            <button type="button" onClick={() => setSaveMode(true)} className="eyebrow inline-flex items-center gap-1 !text-vermilion hover:underline focus-ink">
                  ...
                  {BREAK_ICON_NAMES.map((n) => {
                    ...
                    return (
                      <button
                        ...
                        aria-label={n}
                        className={cn(
                          "aspect-square flex items-center justify-center border rounded-sm transition-colors focus-ink",
                          active ? "border-ink bg-ink text-parchment" : "border-rule text-ink hover:border-ink/50"
                        )}
                      >
