...
**Off by default, and it stays off unless an owner turns it on.** There is no
path in PTD that reports anything about your installation without an explicit
opt-in, and nothing about it is enabled by upgrading.

Two independent switches live in **Org → Data → Installation telemetry**
(owner-only, because the setting belongs to the installation rather than to one
organization):

- **Submit a weekly install ping** — posts exactly four fields, once a week.
- **Check for updates** — a bare `GET` against the public releases feed. Sends no
  payload whatsoever.

### What is sent

The whole payload, with nothing accompanying it beyond a `Content-Type` header —
...
| Field | Source |
|---|---|
| `installation_id` | 16 random bytes as hex, minted once on first boot and kept in the install record. Opaque; not derived from anything |
| `dashboard_version` | `ptd/` plus `PTD_VERSION` (or `package.json`). The receiver is shared with the other products in the family, so the `ptd/` tag is what tells them apart |
| `heliosdb_version` | `SELECT version()` parsed for the HeliosDB-Nano version. `null` on stock Postgres |
| `timestamp` | `new Date().toISOString()` at submit time |

It does **not** send your IP, username, email, hostname, OS or architecture,
organization or stream names, any counts, or any contents of your tasks, time
entries, invoices or attachments. The section shows the exact JSON above before
anything is sent, generated by the same function that does the posting — so what
you read is what would leave the host.

### How to opt in, and out

Org → Data → Installation telemetry, either checkbox. Or from the CLI / MCP:

```bash
ptd action telemetry.status                          # read, sends nothing
ptd action telemetry.set '{"telemetryEnabled":true}' # opt in  (audited)
ptd action telemetry.set '{"telemetryEnabled":false}'# opt out (audited)
ptd action telemetry.ping                            # send one now
```

Turning it off stops the timer immediately; nothing further is sent and nothing
already counted can be traced back (see retention below).

| Variable | Default | Meaning |
|---|---|---|
| `PTD_TELEMETRY` | *(empty)* | `0` forces the ping off and `1` forces it on, outranking the stored toggle — for settling the question across a fleet from the env file. Anything else leaves the UI in charge |
| `PTD_TELEMETRY_ENDPOINT` | `https://telemetry.danimoya.com/v1/ping` | Where the ping goes. Point it at your own receiver, or anywhere, and the default is never contacted |
| `PTD_UPDATE_CHECK_URL` | `https://api.github.com/repos/danimoya/ptd/releases/latest` | The releases feed for the update check. A bare `GET`, no payload |
| `PTD_SCHEDULER` | *(empty)* | `0` disables the weekly ping timer along with the recurrence scheduler |

The install record is a single JSON file at `${PTD_FILES_DIR}/.ptd-install.json`
(mode `0600`, on the `ptd_files` volume) holding the installation id, the two
toggles and the last ping's result. Deleting it mints a new id, which counts as a
new install; copying a volume to a second deployment makes both report the same
id, so mint a fresh one by deleting the file on the clone.

### The offline / air-gapped method

An egress-restricted host cannot reach the receiver, so the section renders the
same payload as a ready-to-paste **URL, curl, wget, HTTPie, PowerShell or plain
JSON** snippet. Copy one, run it from any machine with internet access, and it
counts exactly once: the receiver dedupes on
`SHA-256(salt || ip || installation_id)` per ISO week, so a submission from a
different network still increments this install's count for that week and no
more. The URL form is the one to reach for — paste it into any browser and the
receiver answers a small *Ping received* page.

### Retention at the receiver

Quoted from the receiver's own README, which is published so the question can be
answered definitively rather than promised:

- It hashes `(client_ip, installation_id)` with a rotating server-side salt under
  a `YYYY-WW` bucket key. **The raw IP and the raw `(ip, id)` tuple are never
  persisted** — not logged, not written to disk, not forwarded.
- Only `(week_bucket, hash)` rows are kept, plus the two version columns.
  "Active installs this week" is
  `SELECT COUNT(DISTINCT hash) WHERE week_bucket = current_week`.
- The salt rotates weekly, so hashes from week *N* can never be cross-correlated
  with hashes from week *N+1*.
- **Per-row data is dropped after 90 days. Aggregate weekly counts are kept
  indefinitely.**
- No third-party processors, no analytics SDK, no client-side trackers. The
  endpoint is operated by the PTD maintainer and the data is not resold.

Receiver source — schema, retention job, salt-rotation cron and an append-only
changelog of policy changes — at
[github.com/danimoya/telemetry](https://github.com/danimoya/telemetry). Read it
before you opt in.
