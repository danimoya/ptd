/**
 * The recurrence rule grammar, as a pure module.
 *
 * Deliberately free of imports so the client can carry a byte-identical copy
 * (client/src/features/plan/recurrence.ts) and show the same human preview the
 * scheduler will act on — the same trick server/cascade.ts and its client twin
 * already use. tests/workflow/recurrence.test.ts asserts the two agree.
 *
 *   daily                 every day
